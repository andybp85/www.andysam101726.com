...
```
LOC=$(curl -s -o /dev/null -X POST "$URL" --data-urlencode 'VERB=POST' --data-urlencode 'password=<surname>' -D - | awk 'tolower($1)=="location:"{print $2}' | tr -d '\r')
curl -s -A 'Mozilla/5.0' "$LOC"
```

Verified 2026-06-18:
- [x] POST login (password=<surname>) -> {status:success, token, redirect:/home/}
- [x] GUESTS -> success, 93 groups (reads the new sheet)
- [x] PUT -> {status:success}; row appended to new RSVP sheet + email to <contact email>. NOTE: also sends an email on every RSVP (Code.gs:84 MailApp.sendEmail).
